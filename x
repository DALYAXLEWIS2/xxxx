if game.PlaceId == 2788229376 then
    ...
    UserInputService.InputBegan:Connect(function(input, isProcessed)
        ...
        if input.KeyCode.Name == shared.Global.Keys.Inventory then
            sortInventory()
        end
    end)
    ...
    Player:GetMouse().KeyDown:Connect(function(Key)
        if shared.Global.Macro.Settings.Enabled then
            local keybind = shared.Global.Keys.Macro  
    ...
    Player:GetMouse().KeyUp:Connect(function(Key)
        if shared.Global.Macro.Settings.Mode == 'hold' and Key == shared.Global.Keys.Macro:lower() then
            isHoldingKey = false
    ...
    local Key = shared.Global.Keys.Trigger
    ...
    Mouse.KeyDown:Connect(function(Key)
        ...
        if key == shared.Global.Keys.Target:lower() then
        ...
        if key == shared.Global.Keys.Cancel:lower() then
            IsTargeting = false
            TargetPlayer = nil
        end
    end)
    ...
    game:GetService('RunService').RenderStepped:Connect(function()
        local config = shared.Global.Hitbox
    ...
    UserInputService.InputEnded:Connect(function(input, isProcessed)
        if input.KeyCode == Enum.KeyCode[shared.Global.Keys.Target:upper()] and shared.Global.Camera.Method == "hold" then  -- Updated to Keybind
...
-- Removed the kick if gameSettings is nil
...
-- Removed the kick if MainEvent is not found
...
-- Removed the kick for invalid argument
...
local function updateFov()
    ...
    if IsTargeting then
        if currentWeapon == "db" then
            dynamicFovSize = shared.Global.FOV['DoubleBarrelSG'][1] or 3.5  
        elseif currentWeapon == "rev" then
            dynamicFovSize = shared.Global.FOV['Revolver'][1] or 3.5 
        elseif currentWeapon == "Tactical SG" then
            dynamicFovSize = shared.Global.FOV['TacticalShotgun'][1] or 3.5  
        elseif currentWeapon == "Rifle" then
            dynamicFovSize = shared.Global.FOV['Rifle'][1] or 3.5   
        elseif currentWeapon == "AUG" then
            dynamicFovSize = shared.Global.FOV['AUG'][1] or 3.5  
        end
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player.Character and player ~= Local_Player then
                ...
                if success and not isDead then
                    local success2, closestPart, closestPoint = pcall(function() return GetClosestHitPoint(player.Character) end)
...
-- UPDATED FUNCTION: 
   local function GetClosestHitPoint(character)
    ...
    -- mouse ray origin + direction
    local mouseRay = Current_Camera:ScreenPointToRay(Mouse.X, Mouse.Y)
    ...
    for _, bodyPartName in pairs(AllBodyParts) do
        ...
        if part and part:IsA("BasePart") and isPartInFovAndVisible(part) and isPartVisible(part) then
            -- get the closest point on this part to the mouse ray origin
            local closestOnPart = part:ClosestPoint(mouseRay.Origin)

            -- project to screen
            local screenPoint, onScreen = Current_Camera:WorldToScreenPoint(closestOnPart)
            if onScreen then
                local distance = (V2(screenPoint.X, screenPoint.Y) - V2(Mouse.X, Mouse.Y)).Magnitude
